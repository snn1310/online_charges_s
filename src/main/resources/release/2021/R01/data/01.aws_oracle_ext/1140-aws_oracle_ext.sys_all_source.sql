create or replace view aws_oracle_ext.sys_all_source as
select
...
from (
    ...
    where (
         u.usename = user 
         or
         has_table_privilege(user, c.oid, 'TRIGGER')
       )
    ...
    select 
     ...
     where f.prokind in ('f', 'p')
       and (
         u.usename = user 
         or
         has_function_privilege(user, f.oid, 'EXECUTE')
       )
    union all
  ...
  where (
   ...
   and (
     u.usename = user
     or 
     has_type_privilege(user, t.oid, 'USAGE')
   )
  ) s
