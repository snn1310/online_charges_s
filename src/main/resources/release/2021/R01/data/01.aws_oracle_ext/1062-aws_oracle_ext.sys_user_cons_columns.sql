create or replace view aws_oracle_ext.sys_user_cons_columns AS
select
...
COMMENT ON VIEW aws_oracle_ext.sys_user_cons_columns IS 'USER_CONS_COLUMNS describes columns that are owned by the current user and that are specified in constraint definitions.';
