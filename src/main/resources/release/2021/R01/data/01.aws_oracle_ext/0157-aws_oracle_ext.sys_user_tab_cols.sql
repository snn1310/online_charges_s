create or replace view aws_oracle_ext.sys_user_tab_cols as
select 
	...
	, 'NO'::varchar(3) as HIDDEN_COLUMN
	, 'NO'::varchar(3) as VIRTUAL_COLUMN
	, a.attnum::numeric as SEGMENT_COLUMN_ID
	, a.attnum::numeric as INTERNAL_COLUMN_ID
	, 'NONE'::varchar(4) as HISTOGRAM	
	, a.attname::varchar(4000) as QUALIFIED_COL_NAME
from pg_attribute a
...
comment on view aws_oracle_ext.SYS_USER_TAB_COLS is
'Describes the columns of the tables, views, and clusters owned by the current user. This view does not display the OWNER column.'
