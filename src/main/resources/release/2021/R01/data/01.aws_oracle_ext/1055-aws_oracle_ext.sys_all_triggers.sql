create or replace
view aws_oracle_ext.sys_all_triggers as
select
	n.nspname::character varying(128) as OWNER,
...
and (pg_has_role(tbl.relowner, 'USAGE'::text)
	or has_table_privilege(tbl.oid, 'INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER'::text)
	or has_any_column_privilege(tbl.oid, 'INSERT, UPDATE, REFERENCES'::text));

COMMENT ON VIEW aws_oracle_ext.sys_all_triggers IS 'ALL_TRIGGERS describes the triggers on tables accessible to the current user.';
