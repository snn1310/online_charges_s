CREATE TABLE ocentitlements.audit_log(
...
COMMENT ON COLUMN ocentitlements.audit_log.al_audit_id
     IS 'Audit log entry guid.';
COMMENT ON COLUMN ocentitlements.audit_log.al_api
     IS 'Web service api.';
COMMENT ON COLUMN ocentitlements.audit_log.al_app_environment
     IS 'Environment hosting web service.';
COMMENT ON COLUMN ocentitlements.audit_log.al_change_action
     IS 'Identifies change as update or delete.';
COMMENT ON COLUMN ocentitlements.audit_log.al_primary_key
     IS 'List of values from primary key of changed data.';
COMMENT ON COLUMN ocentitlements.audit_log.al_root_guid
     IS 'Root guid of request that generated audit log entry.';
COMMENT ON COLUMN ocentitlements.audit_log.al_table_name
     IS 'Database name of table that contained original data.';
COMMENT ON COLUMN ocentitlements.audit_log.al_data
     IS 'Pre-change data object.';
COMMENT ON COLUMN ocentitlements.audit_log.al_modified_by
     IS 'User or system performing modification.';
COMMENT ON COLUMN ocentitlements.audit_log.al_mod_timestamp
